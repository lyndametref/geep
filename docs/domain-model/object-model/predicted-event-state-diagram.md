...
Predicted Event is a specialization of Future Event representing a probabilistic outcome with an expected date range (e.g., a predicted lambing window from a mating observation). It uses the `PredictionStatus` enum with three lifecycle states.
...
| `Aborted` | The predicted event will not occur (e.g., the user confirms the animal did not lamb within the expected window). Terminal state. |
