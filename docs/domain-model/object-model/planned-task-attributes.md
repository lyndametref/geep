...
| `sourceRecordId` | Long | No | Logical FK to the Record that derived this PlannedTask. | BR-012, BR-013, BR-014 |
  | `createdAt` | Timestamp (UTC) | Yes | ISO-8601 UTC timestamp. When the PlannedTask was derived or manually created. | REQ-04, REQ-05.002, BR-016 |
| `futureEventType` | Enum | Yes | Fixed to `PLANNED_TASK` for PlannedTask instances. | BR-010 |

## PlannedTask-specific attributes

| Attribute | Type | Required | Constraints / Format | Requirement |
|-----------|------|----------|----------------------|-------------|
| `status` | Enum (TaskStatus) | Yes | Lifecycle status. Values: `PENDING` (awaiting completion), `DONE` (completed), `CANCELLED` (no longer required). | BR-010, BR-015 |
